...
- [session_test.go](file://internal/core/workflow/session_test.go)
...
- 执行上下文：internal/core/workflow/context.go
- 数据库Schema与索引：internal/infrastructure/db/migrations/001_init_schema.up.sql
- 数据库接口与连接池：internal/infrastructure/db/interface.go、internal/infrastructure/db/postgres.go
- 工作流持久化仓库（参考实现）：internal/infrastructure/persistence/workflow_repository.go
...
Ctx["WorkflowContext<br/>执行上下文"]
end
subgraph "数据库"
DBI["DB 接口"]
PG["Postgres 连接池"]
MIG["迁移脚本<br/>sessions/memories/session_messages"]
end
subgraph "前端类型"
...
- [001_init_schema.up.sql](file://internal/infrastructure/db/migrations/001_init_schema.up.sql#L56-L96)
...
- WorkflowContext：执行上下文，携带SessionID与全局只读输入
...
- 数据层通过迁移脚本建立sessions、session_messages、memories三张表，并建立外键关系
...
图表来源
- [session.go](file://internal/core/workflow/session.go#L12-L40)
- [session.go](file://internal/core/workflow/session.go#L51-L165)

章节来源
- [session.go](file://internal/core/workflow/session.go#L12-L165)
- [session_test.go](file://internal/core/workflow/session_test.go#L1-L116)
...
- 启动会话
  - 路径：NewSession -> Start
  - 示例位置：[session.go](file://internal/core/workflow/session.go#L42-L62)
- 更新状态（完成）
  - 路径：Complete
  - 示例位置：[session.go](file://internal/core/workflow/session.go#L64-L72)
- 更新状态（失败/取消）
  - 路径：Stop
  - 示例位置：[session.go](file://internal/core/workflow/session.go#L144-L155)
- 存储提案（proposal）
...
章节来源
- [session.go](file://internal/core/workflow/session.go#L74-L165)
- [001_init_schema.up.sql](file://internal/infrastructure/db/migrations/001_init_schema.up.sql#L56-L96)
- [postgres.go](file://internal/infrastructure/db/postgres.go#L1-L65)

## 结论
Session实体模型清晰地将一次工作流执行抽象为可追踪、可观测、可并发控制的实例。通过明确的状态机、严谨的数据库外键与索引设计，以及与消息、记忆表的紧密关联，支撑了从执行到回放的全链路能力。配合前端类型定义，实现了前后端一致的会话状态语义。
...
- 迁移脚本参考
  - sessions、session_messages、memories建表与索引
  - 示例位置：[001_init_schema.up.sql](file://internal/infrastructure/db/migrations/001_init_schema.up.sql#L56-L96)
- 下游脚本（不影响会话模型）
  - [001_init_schema.down.sql](file://internal/infrastructure/db/migrations/001_init_schema.down.sql#L1-L96)
  - [002_add_quarantine_logs.up.sql](file://internal/infrastructure/db/migrations/002_add_quarantine_logs.up.sql#L1-L96)
  - [004_create_llm_options.up.sql](file://internal/infrastructure/db/migrations/004_create_llm_options.up.sql#L1-L96)