...
classDiagram
    class Engine {
        ...
        +executeNode(ctx context.Context, nodeID string, input Map)
        +handleParallel(ctx context.Context, node *Node, input Map)
    }
    class NodeProcessor {
        <<interface>>
        +Process(ctx context.Context, input Map, stream chan StreamEvent) : Map
    }
